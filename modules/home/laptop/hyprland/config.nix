{ pkgs, lib, config, ... }:
{
  # hyprmon (Super+S -> Display Settings) checks for an exact-match
  # `require("hyprmon")` line before it'll skip rewriting hyprland.lua (which
  # would fail: that file is our immutable home-manager symlink). It only
  # ever adds that line right after first writing hyprmon.lua itself, so on a
  # machine that's never run hyprmon yet the require below would error at
  # Hyprland startup. Seed an empty sidecar once, without ever clobbering
  # hyprmon's own saved layout on later rebuilds.
  ...
  wayland.windowManager.hyprland = {
    extraConfig = ''
      ...
      -- Load whatever layout the hyprmon TUI (Super+S -> Display Settings) last
      -- saved, overriding the fallback above. hyprmon.lua is a plain file it
      -- manages itself (not home-manager), so this survives both dock/undock
      -- changes and `home-manager switch`. Must be this exact line (hyprmon
      -- checks for it verbatim) so hyprmon never tries to rewrite this
      -- (immutable, Nix-owned) file itself.
      ...
      hl.bind(mainMod .. " + SHIFT + D",      hl.dsp.exec_cmd("hyprctl workspaces -j | jq -r '.[] | select(.monitor == \"eDP-1\") | .id' | xargs -I{} hyprctl dispatch moveworkspacetomonitor {} +1 && hyprctl dispatch moveworkspacetomonitor 1 +1"))
      hl.bind(mainMod .. " + SHIFT + E",      hl.dsp.exec_cmd("if [ -f /tmp/externals-disabled ]; then hyprctl keyword monitor ,preferred,auto,1 && rm /tmp/externals-disabled; else hyprctl keyword monitor ,disabled && touch /tmp/externals-disabled; fi"))
