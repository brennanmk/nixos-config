...
hl.bind(
	mainMod .. " + SHIFT + E",
	hl.dsp.exec_cmd(
		"if [ -f /tmp/externals-disabled ]; then hyprctl keyword monitor ,preferred,auto,1 && rm /tmp/externals-disabled; else hyprctl keyword monitor ,disabled && touch /tmp/externals-disabled; fi"
	)
)
...
hl.bind(
	mainMod .. " + SHIFT + D",
	hl.dsp.exec_cmd(
		"hyprctl workspaces -j | jq -r '.[] | select(.monitor == \"eDP-1\") | .id' | xargs -I{} hyprctl dispatch moveworkspacetomonitor {} +1 && hyprctl dispatch moveworkspacetomonitor 1 +1"
	)
)
