...
PanelWindow {
    ...
    // Follows the monitor Hyprland currently considers focused, which
    // (with follow_mouse enabled) tracks the monitor the cursor is on.
    screen: {
    ...
    implicitWidth: 400
    implicitHeight: 1000
    // Don't cover fullscreen apps (games, video players, etc) with toasts.
    visible: !(Hyprland.focusedMonitor && Hyprland.focusedMonitor.activeWorkspace && Hyprland.focusedMonitor.activeWorkspace.hasFullscreen)
    color: "transparent"

    // Only the monitor the bar lives on needs room to clear it; other
    // monitors have no bar, so a big top gap there just looks like an
    // unexplained offset.
    readonly property string barMonitorName: Quickshell.env("QS_PRIMARY_MONITOR")
    readonly property bool onBarMonitor: !barMonitorName || (root.screen && root.screen.name === barMonitorName)
