...
QuickSettingsTile {
    ...
    function connect(ssid, password) {
        wifiConnectProc.command = password ? ["nmcli", "device", "wifi", "connect", ssid, "password", password] : ["nmcli", "device", "wifi", "connect", ssid];
        wifiConnectProc._ssid = ssid;
        wifiConnectProc._error = "";
    ...
    Process {
        ...
        onExited: function(exitCode) {
            if (exitCode !== 0) {
                root.connectFailed(wifiConnectProc._ssid, wifiConnectProc._error.trim() || "Failed to connect");
            } else {
                root.connectSucceeded(wifiConnectProc._ssid);
                scan();
            }
        }
    }
