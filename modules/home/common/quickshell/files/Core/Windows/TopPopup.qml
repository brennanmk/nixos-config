...
Item {
    ...
    // When content can resize itself while open (e.g. async network status
    // text changing a row's implicit width), animating smoothWidth/Height
    // causes overflow: Layout.fillWidth children never shrink below their
    // natural minimum, so content renders at full size even while the
    // container is still mid-grow-animation. Disable for such popups.
    property bool animateSize: true
    property bool alignRight: false
    property bool squareRightCorners: false
    ...
    Item {
        ...
        width: smoothWidth
        height: smoothHeight
        x: root.alignRight ? (root.width - smoothWidth) : root.overshootHeadroom
        y: 0
        ...
        Shape {
            ...
            readonly property real rl: r
            readonly property real rr: root.squareRightCorners ? 0 : r
        ...
        ColumnLayout {
            ...
            readonly property real leftInset: root.contentPadding + root.cornerRadius
            readonly property real rightInset: root.squareRightCorners ? root.contentPadding : (root.contentPadding + root.cornerRadius)

            x: leftInset
            y: root.contentPadding
            width: smoothWidth - leftInset - rightInset
        }

    }

    Behavior on smoothHeight {
        enabled: HyprlandService.enableAnimations && root.isOpen && root.animateSize
