...
"""
    ...
    # sed 's/^key=.*/key=val/' only replaces a line that's already there --
    # home-manager's generated settings.ini never had gtk-theme-name or
    # prefer-dark-theme keys to begin with, so that was always a silent
    # no-op. Set them properly (Adwaita's dark variant is what actually
    # gives GTK3 apps like Nemo real dark widget chrome; the custom
    # "Material-Gnome" theme only carries color variables, not full CSS).
    def set_ini_keys(path, updates):
        lines = []
        ...
        if remaining:
            insert_at = next((i + 1 for i, l in enumerate(lines) if l.strip() == "[Settings]"), len(lines))
            for key, val in remaining.items():
                lines.insert(insert_at, f"{key}={val}")
                insert_at += 1

        os.makedirs(os.path.dirname(path), exist_ok=True)
        # home-manager manages settings.ini as a symlink into the read-only
        # Nix store; writing through it raises "Read-only file system".
        # Drop the symlink so we can lay down a real, mutable file.
    ...
    gtk_updates = {
        ...
        "gtk-application-prefer-dark-theme": "true" if is_dark else "false",
    }
    set_ini_keys(os.path.join(home, ".config", "gtk-3.0", "settings.ini"), gtk_updates)
    set_ini_keys(os.path.join(home, ".config", "gtk-4.0", "settings.ini"), gtk_updates)
        ...
        for svg_file in ["settings-icon.svg", "keybinds-icon.svg"]:
            ...
            if content != new_content:
                try:
                    ...
                    # ~/.config/quickshell/assets is a read-only Nix store
                    # symlink under home-manager; this cosmetic recolor step
                    # just can't apply there. Everything else above already
                    # succeeded, so don't let this abort the whole script.
                    pass
